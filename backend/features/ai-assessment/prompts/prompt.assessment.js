...
Return VALID JSON ONLY. No markdown. No extra keys. One object only.
{
  ...
  "operations": "Use the Five-Part Plan format. Each part must start on its own line, beginning with its label in bold followed by a colon. Use this exact structure:\nSituation: [crime pattern from CPA]\nMission: [one sentence goal]\nExecution: [patrol method matched to place type, cluster barangay by name if detected, peak hours]\nTasks (MUST DOs): [write exactly 2 MUST DOs as plain sentences, each starting with a time in AM/PM format, e.g. 'At 8:00 AM – 9:00 AM, ...', tied to ACTUAL MODUS IN DATA]\nCoordinating Instructions: [name relevant force multipliers]\nDo NOT use arrays, bullet points, or JSON inside this field. Each label and its content must be on the same line, separated from the next section by a newline character.",
