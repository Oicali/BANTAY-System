...
const HEATMAP_LAYER = {
  ...
  paint: {
    ...
    "heatmap-color": [
      ...
      ["heatmap-density"],
      0,    "rgba(0,0,0,0)",
      0.10, "rgba(0,80,0,0.6)",
      0.25, "rgba(34,255,68,0.75)",
      0.45, "rgba(255,255,0,0.82)",
      0.65, "rgba(255,102,0,0.88)",
      0.82, "rgba(220,20,20,0.92)",
      1.0,  "rgba(10,0,0,0.96)",
    ],
    "heatmap-opacity": [
      "interpolate", ["linear"],
      ["zoom"],
...
function CrimeMapping() {
  ...
  const [filters,         setFilters]         = useState({
    incident_type: "", date_from: "", date_to: "", barangay: "",
  });
    ...
    <div className="crmap-wrapper">
      ...
      <div className="crmap-filterbar">
        <div className="crmap-filterbar-inner">
              ...
              if (selected && geoJSONData) {
                ...
                if (feature && mapRef.current) {
                  ...
                  mapRef.current.flyTo({
                    ...
                    zoom: 15, duration: 1200,
                  });
                }
              } else if (!selected && mapRef.current) {
                mapRef.current.flyTo({
                  center: [120.9640, 14.4341], zoom: 12, duration: 1200,
                });
              }
            }}>
            <option value="">All Barangays</option>
            {CURRENT_BARANGAYS.map(b => (
              <option key={b} value={b}>{formatBarangayLabel(b)}</option>
            ))}
            <optgroup label="── Pre-2023 Names (Auto-resolved) ──">
              {LEGACY_BARANGAY_OPTIONS.map((b, idx) => (
                <option key={`legacy-${idx}`} value={b.value}>{b.label}</option>
              ))}
            </optgroup>
          </select>

          <div className="crmap-date-range">
            <input type="date" className="crmap-fsel crmap-fsel-date"
          ...
          <button className="crmap-apply-btn" onClick={() => {
          ...
          <button className="crmap-clear-btn" onClick={() => {
            ...
            mapRef.current?.flyTo({ center: [120.9640, 14.4341], zoom: 12, duration: 800 });
          }}>
            Clear
          </button>
        </div>
        ...
        <div className="crmap-map-wrap">
          ...
          {error && (
            <div className="crmap-error-banner">
              ...
              {error}
              <button onClick={() => heatmapMode ? fetchHeatmap() : fetchAll()}>
                Retry
              </button>
            </div>
          )}
            ...
            >
              ...
              {!heatmapMode && showPins && zoom >= 13 && pins.map(pin => (
                ...
                >
                  <div
                    className="crmap-pin"
                    style={{
                      background: INCIDENT_COLORS[pin.incident_type?.toUpperCase()] || "#6b7280",
                    }}
              ...
              {!heatmapMode && selectedPin && (
                <Popup
                  ...
                  anchor="bottom"
                  onClose={() => { setSelectedPin(null); setShowMorePopup(false); }}
                  closeOnClick={false}
                  maxWidth="290px"
                >
                  <div className="crmap-popup">
                    <div
                      ...
                      style={{
                        background:
                          INCIDENT_COLORS[selectedPin.incident_type?.toUpperCase()] || "#495057",
                      }}
                    >
                  ...
                  <div className="crmap-popup">
                    ...
                    <div className="crmap-popup-body">
                      {[
                        ...
                        ["Crime types", Array.isArray(selectedCluster.crime_types)
          ...
          <div className="crmap-controls">
            ...
            <button className="crmap-ctrl-btn" title="Reset view"
              onClick={() => mapRef.current?.flyTo({
                center: [120.9640, 14.4341], zoom: 12, duration: 800,
              })}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" strokeWidth="2">
          ...
          <button
            ...
            title={heatmapMode ? "Switch to choropleth view" : "Switch to heatmap view"}
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none"
                ...
                <div className="crmap-panel-section">
                  {heatmapMode ? (
                    <div className="crmap-heat-sidebar-legend">
                      <div className="crmap-heat-sidebar-title">Density scale</div>
                      <div className="crmap-heat-scale-bar" />
                      <div className="crmap-heat-scale-labels">
                        <span>Low</span><span>High</span>
                      </div>
                      <div className="crmap-heat-sidebar-title" style={{ marginTop: 16 }}>
                        Severity weight
                      </div>
                      {[
                        ...
                        <div key={r.crime} className="crmap-severity-row">
                          <span className="crmap-severity-crime">{r.crime}</span>
                    ...
                    ? stats.recent_incidents.map((r, i) => (
                      <div className="crmap-recent-item" key={i}>
                        <div className="crmap-recent-bar"
                          style={{
                            background: INCIDENT_COLORS[r.incident_type?.toUpperCase()] || "#6b7280",
                          }} />
