...
const AddPatrolModal = ({ mobileUnits, geoJSONData, onClose, onSave }) => {
  ...
  useEffect(() => {
    ...
    fetch(
      `${API_BASE}/patrol/available-patrollers?start=${form.start_date}&end=${form.end_date}`,
      {
        headers: {
      ...
      .then((data) => {
        ...
        if (!data.success) {
          setNotif({
            message: "Could not check patroller availability. Please retry.",
            type: "error",
          });
          return;
        }
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .then((data) => {
        ...
        if (!data.success) {
          setNotif({
            message: "Could not check mobile unit availability. Please retry.",
            type: "error",
          });
          return;
        }
  ...
  const currentSelectedIds =
    activeShift === "AM" ? selectedAMIds : selectedPMIds;
  const setCurrentSelectedIds =
    activeShift === "AM" ? setSelectedAMIds : setSelectedPMIds;
  ...
  const togglePatroller = (id) => {
    if (otherShiftIds.includes(id)) {
      setNotif({
        message: `This patroller is already assigned to the ${activeShift === "AM" ? "PM" : "AM"} shift.`,
        type: "warning",
      });
      return;
    }
  ...
  const handleSave = () => {
    ...
    if (barangays.length === 0) {
      setNotif({
        message: "Please select at least one barangay on the map.",
        type: "warning",
      });
      return;
    }
    if (selectedAMIds.length === 0 && selectedPMIds.length === 0) {
      setNotif({
        message: "Please assign at least one patroller to AM or PM shift.",
        type: "warning",
      });
      return;
    }
    ...
    for (const task of tasks) {
      ...
      const startMin = task.time_start
        .split(":")
        .map(Number)
        .reduce((h, m) => h * 60 + m);
      const endMin = task.time_end
        .split(":")
        .map(Number)
        .reduce((h, m) => h * 60 + m);
      ...
      const effectiveStart =
        task.shift === "PM" ? toPmMin(task.time_start) : startMin;
      const effectiveEnd =
        task.shift === "PM" ? toPmMin(task.time_end) : endMin;
        ...
        <div className="apm-topbar">
          <div className="apm-topbar-fields">
            <div className="apm-field">
              <label>
                Patrol Name <span className="apm-req">*</span>
              </label>
              <input
                type="text"
            ...
            <div className="apm-field">
              <label>
                Mobile Unit <span className="apm-req">*</span>
              </label>
              <select
                ...
                disabled={
                  !form.start_date || !form.end_date || loadingMobileUnits
                }
              >
                {!form.start_date || !form.end_date ? (
            ...
            <div className="apm-field">
              <label>
                Start Date <span className="apm-req">*</span>
              </label>
              <input
                type="date"
            ...
            <div className="apm-field">
              <label>
                End Date <span className="apm-req">*</span>
              </label>
              <input
                type="date"
                value={form.end_date}
                ...
                onChange={(e) => {
                  ...
                  if (
                    diffDaysInclusive(form.start_date, newEnd) > MAX_PATROL_DAYS
                  ) {
                    setNotif({
          ...
          <div className="apm-topbar-actions">
            ...
            <button className="apm-btn-x" onClick={onClose}>
              ✕
            </button>
          </div>
        </div>
          ...
          <div className="apm-info-panel">
            ...
            <div className="apm-shift-tabs-top">
              ...
              >
                ...
                {selectedAMIds.length > 0 && (
                  <span className="apm-shift-badge">
            ...
            <div className="apm-section">
              <div className="apm-section-title-row">
                <div className="apm-section-title">
                  ...
                  {currentSelectedIds.length > 0 && (
                    <span className="apm-shift-badge" style={{ marginLeft: 6 }}>
                ...
                {availableForDates !== null &&
                  ...
                  (showPatrollers ? (
                    <button
                      className="apm-toggle-btn apm-toggle-hide"
                      onClick={() => {
                        setShowPatrollers(false);
                        setPatrollerPage(1);
                      }}
                    ...
                    <button
                      className="apm-toggle-btn apm-toggle-show"
              ...
              {availableForDates === null ? (
                ...
                >
                  Please select a start and end date to see available
                  patrollers.
                </p>
              ) : loadingPatrollers ? (
                          ...
                          <div className="apm-checklist">
                            {filtered.length === 0 ? (
                              <div className="apm-empty">
                                No available patrollers for this period.
                              </div>
                            ) : (
                              paged.map((p) => {
                                ...
                                return (
                                  <div
                                    ...
                                    title={
                                      isOtherShift
                                        ? `Already assigned to ${activeShift === "AM" ? "PM" : "AM"} shift`
                                        : ""
                                    }
                                  >
                                    <div
                                    ...
                                    <div className="apm-officer-info">
                                      <span className="apm-officer-name">
                                        {p.officer_name}
                                      </span>
                                      {isOtherShift && (
                                    ...
                                    <div className="apm-checkbox-col">
                                      <div
                                        className={`apm-checkbox ${isSelected ? "apm-checkbox-on" : ""}`}
                                      >
                                        {isSelected ? "✓" : ""}
                                      </div>
                                    </div>
            ...
            <div className="apm-section apm-section-grow">
                            ...
                            <td className="apm-tt-time">
                              <div className="apm-time-inputs">
                                <TimePicker
                                  ...
                                  onChange={(v) =>
                                    updateTask(task._id, "time_start", v)
                                  }
                                ...
                                <TimePicker
                                  ...
                                  onChange={(v) =>
                                    updateTask(task._id, "time_end", v)
                                  }
                                  onBlur={sortCurrentShift}
                                  baseHour={
                                    task.time_start
              ...
              <button className="apm-add-task-btn" onClick={addTask}>
                + Add Task
              </button>
            </div>
          </div>
